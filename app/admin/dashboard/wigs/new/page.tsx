...
export default function NewWigPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        formData.append('folder', 'luxuryher/wigs');
  ...
  const handleVideoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('folder', 'luxuryher/videos');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setToast({ message: 'Wig created successfully!', type: 'success' });
        setTimeout(() => {
          router.push('/admin/dashboard/wigs');
        }, 1000);
      } else {
        const data = await res.json();
        setToast({ message: data.error || 'Failed to create wig', type: 'error' });
      }
    } catch (error) {
      setToast({ message: 'Failed to create wig', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Add New Wig</h1>
        <p className="text-gray-600">Create a new wig listing</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center space-x-4 pt-6 border-t">
            ...
            >
              ...
              <span>{loading ? 'Creating...' : 'Create Wig'}</span>
            </button>
            <Link
              href="/admin/dashboard/wigs"
